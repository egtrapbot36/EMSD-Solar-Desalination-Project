...
Adafruit_MQTT_Subscribe RelayAcid  = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/relay_acid");
Adafruit_MQTT_Subscribe RelayBase  = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/relay_base");
Adafruit_MQTT_Subscribe RelayNA    = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/relay_nA");
Adafruit_MQTT_Subscribe RelayNB    = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/relay_nB");
Adafruit_MQTT_Subscribe RelaySalt  = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/relay_salt");
Adafruit_MQTT_Subscribe RelayFresh = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/relay_fresh");
...
void activatecallback_relay_acid(char *data, uint16_t len) {
  relay_acid = String(data);
}

//updates Relay 2 State based on Adafruit.io Motor2 feed
void activatecallback_relay_base(char *data, uint16_t len) {
  relay_base = String(data);
}

//updates Relay 3 State based on Adafruit.io Motor3 feed
void activatecallback_relay_nA(char *data, uint16_t len) {
  relay_nA = String(data);
}

//updates Relay 4 State based on Adafruit.io Motor4 feed
void activatecallback_relay_nB(char *data, uint16_t len) {
  relay_nB = String(data);
}

//updates Relay 4 State based on Adafruit.io Motor4 feed
void activatecallback_relay_salt(char *data, uint16_t len) {
  relay_salt = String(data);
}

//updates Relay 4 State based on Adafruit.io Motor4 feed
void activatecallback_relay_fresh(char *data, uint16_t len) {
  relay_fresh = String(data);
}
...
//  WiFi.begin(WLAN_SSID, WLAN_PASS);
  ...
  RelayAcid.setCallback(activatecallback_relay_acid);
  RelayBase.setCallback(activatecallback_relay_base);
  RelayNA.setCallback(activatecallback_relay_nA);
  RelayNB.setCallback(activatecallback_relay_nB);
  RelaySalt.setCallback(activatecallback_relay_salt);
  RelayFresh.setCallback(activatecallback_relay_fresh);

  // Setup MQTT subscription for feeds.
  mqtt.subscribe(&RelayAcid);
  mqtt.subscribe(&RelayBase);
  mqtt.subscribe(&RelayNA);
  mqtt.subscribe(&RelayNB);
  mqtt.subscribe(&RelaySalt);
  mqtt.subscribe(&RelayFresh);
